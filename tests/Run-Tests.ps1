...
foreach($suite in @('KeyboardTests','IdleGamepadTests','ModifierSafetyPolicyTests','ReleasePolicyTests','UpdateInstallerTests')) {
  ...
  foreach($file in @('InputStitch.cs','ReleaseInfo.cs','VirtualKeyboard.cs','IdleGamepad.cs','UpdateInstaller.cs')) { $arguments += Join-Path $project $file }
