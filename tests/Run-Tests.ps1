...
foreach($suite in @('KeyboardTests','IdleGamepadTests','ModifierSafetyPolicyTests','ReleasePolicyTests')) {
  ...
  foreach($file in @('InputStitch.cs','ReleaseInfo.cs','VirtualKeyboard.cs','IdleGamepad.cs')) { $arguments += Join-Path $project $file }
