...
foreach($suite in @('KeyboardTests','IdleGamepadTests','ModifierSafetyPolicyTests','ReleasePolicyTests','UpdateInstallerTests','UiSafetyTests','ProductivityTests')) {
  ...
  foreach($file in @('InputStitch.cs','ReleaseInfo.cs','VirtualKeyboard.cs','IdleGamepad.cs','UpdateInstaller.cs','RuntimeDiagnostics.cs','ConfigStore.cs','StepHistory.cs','QuickCreate.cs')) { $arguments += Join-Path $project $file }
