...
namespace InputStitch
{
    ...
    public static class Localizer
    {
        ...
        private static readonly Dictionary<string, string> En = new Dictionary<string, string>()
        {
            ...
            { "按一下", "Press" },
            { "保持到宏停止 / KeyDown", "Hold until macro stops / KeyDown" },
    ...
    public static class AppLog
    {
        ...
        private static void RotateIfNeeded()
        {
            ...
            string first = LogPath + ".1";
            string second = LogPath + ".2";
    ...
    public static class GamepadVector
    {
        // UI convention: 0 degrees is forward, +90 right, -90 left and +/-180 back.
        // The execution/configuration layer remains X/Y based for backward compatibility.
        public static void ToCartesian(int angle, int strength, out int x, out int y)
        {
            angle = Math.Max(-180, Math.Min(180, angle));
            strength = Math.Max(0, Math.Min(100, strength));
            double radians = angle * Math.PI / 180.0;
            x = (int)Math.Round(strength * Math.Sin(radians), MidpointRounding.AwayFromZero);
            y = (int)Math.Round(strength * Math.Cos(radians), MidpointRounding.AwayFromZero);
            x = Math.Max(-100, Math.Min(100, x));
            y = Math.Max(-100, Math.Min(100, y));
        }

        public static void FromCartesian(int x, int y, out int angle, out int strength)
        {
            x = Math.Max(-100, Math.Min(100, x));
            y = Math.Max(-100, Math.Min(100, y));
            if (x == 0 && y == 0)
            ...
            angle = (int)Math.Round(Math.Atan2(x, y) * 180.0 / Math.PI, MidpointRounding.AwayFromZero);
            strength = (int)Math.Round(Math.Min(100.0, Math.Sqrt((double)x * x + (double)y * y)), MidpointRounding.AwayFromZero);
        }
    }
    ...
    public static class InputNames
    {
        ...
        public static string FormatInput(InputSpec input)
        {
            ...
            if (input.GamepadControl == GamepadControl.LeftStick || input.GamepadControl == GamepadControl.RightStick)
            {
                int angle;
                int strength;
                GamepadVector.FromCartesian(input.GamepadX, input.GamepadY, out angle, out strength);
                return name + "  " + Localizer.T("方向=") + angle.ToString() + "°  " + Localizer.T("力度=") + strength.ToString() + "%";
            }
        ...
        public static string FormatAction(MacroAction action)
        {
            ...
            if (action == MacroAction.Down) return Localizer.T("保持到宏停止 / KeyDown");
    ...
    {
        private static readonly object Sync = new object();
        private static IVirtualGamepadBackend backend = new VigemVirtualGamepadBackend();
        ...
        {
            get { lock (Sync) return backend.IsConnected ? backend.ConnectedType : ""; }
        }

        public static bool IsConnected
        {
            get { lock (Sync) return backend.IsConnected; }
        }

        public static int OwnXboxUserIndex
        {
            get { lock (Sync) return backend.OwnXboxUserIndex; }
        }
        ...
        public static void EnsureConnected(string requestedType)
        {
            ...
            lock (Sync)
            {
                ...
                if (backend.IsConnected && string.Equals(backend.ConnectedType, normalized, StringComparison.OrdinalIgnoreCase)) return;
                backend.Disconnect();
                try
                {
                    backend.Connect(normalized);
                }
                catch (GamepadOutputException)
                {
                    backend.Disconnect();
                    throw;
                }
                ...
                {
                    backend.Disconnect();
                    throw new GamepadOutputException(GamepadFailureKind.Other, ex.Message, ex);
                }
            }
        }
        ...
        public static void Send(InputSpec input, bool down)
        {
            ...
            lock (Sync)
            {
                if (!backend.IsConnected) EnsureConnected(preferredType);
                try
                {
                    backend.Send(input, down);
                }
                catch (Exception ex)
                {
                    GamepadOutputException failure = ex as GamepadOutputException ??
                        new GamepadOutputException(GamepadFailureKind.Other, ex.Message, ex);
        ...
        {
            lock (Sync) backend.Disconnect();
        }

        internal static IDisposable OverrideBackendForTests(IVirtualGamepadBackend replacement)
        {
            if (replacement == null) throw new ArgumentNullException("replacement");
            lock (Sync)
            {
                if (backend.IsConnected) throw new InvalidOperationException("Disconnect the virtual gamepad before replacing its backend.");
                IVirtualGamepadBackend previous = backend;
                backend = replacement;
                return new BackendOverride(previous, replacement);
            }
        }

        private sealed class BackendOverride : IDisposable
        {
            private readonly IVirtualGamepadBackend previous;
            private readonly IVirtualGamepadBackend replacement;
            private bool disposed;

            internal BackendOverride(IVirtualGamepadBackend previousBackend, IVirtualGamepadBackend replacementBackend)
            {
            ...
            public void Dispose()
            {
                lock (Sync)
                {
                    if (disposed) return;
                    disposed = true;
                    if (!object.ReferenceEquals(backend, replacement)) return;
                    try { replacement.Disconnect(); } catch { }
                    backend = previous;
                }
            }
        }
    ...
    public class StepEditDialog : Form
    {
        ...
        private Label stickAngleLabel;
        private Label stickStrengthLabel;
        private Label triggerValueLabel;
        private NumericUpDown stickAngleBox;
        private NumericUpDown stickStrengthBox;
        private NumericUpDown triggerValueBox;
        private GamepadPreviewControl gamepadPreview;
        ...
        public StepEditDialog(MainForm owner, MacroStep source)
        {
            ...
            actionBox.Items.AddRange(new object[] { Localizer.T("按一下（按下→等待→松开）"), Localizer.T("保持到宏停止 / KeyDown"), Localizer.T("松开 / KeyUp") });
            ...
            inputTypeBox.SelectedIndexChanged += delegate { if (!loadingControls) UpdateInputMode(); };
            ...
            gamepadControlBox.SelectedIndexChanged += delegate { if (!loadingControls) UpdateGamepadControl(); };
            gamepadPanel.Controls.Add(gamepadControlBox);

            stickAngleLabel = MakeInlineLabel(Localizer.T("摇杆方向："));
            gamepadPanel.Controls.Add(stickAngleLabel);
            stickAngleBox = MakeNumberBox(-180, 180, 0, 5);
            stickAngleBox.ValueChanged += GamepadVectorChanged;
            gamepadPanel.Controls.Add(stickAngleBox);
            stickStrengthLabel = MakeInlineLabel(Localizer.T("摇杆力度："));
            gamepadPanel.Controls.Add(stickStrengthLabel);
            stickStrengthBox = MakeNumberBox(0, 100, 100, 5);
            stickStrengthBox.ValueChanged += GamepadVectorChanged;
            gamepadPanel.Controls.Add(stickStrengthBox);
            triggerValueLabel = MakeInlineLabel(Localizer.T("扳机力度："));
            gamepadPanel.Controls.Add(triggerValueLabel);
            triggerValueBox = MakeNumberBox(0, 100, 100, 5);
            triggerValueBox.ValueChanged += TriggerValueChanged;
            gamepadPanel.Controls.Add(triggerValueBox);
            root.Controls.Add(gamepadPanel, 1, 3);
            root.SetColumnSpan(gamepadPanel, 2);

            gamepadPreview = new GamepadPreviewControl();
            gamepadPreview.Dock = DockStyle.Fill;
            gamepadPreview.Margin = new Padding(16, 0, 0, 8);
            root.Controls.Add(gamepadPreview, 3, 0);
            root.SetRowSpan(gamepadPreview, 7);
            ...
            int initialAngle;
            int initialStrength;
            GamepadVector.FromCartesian(selectedInput.GamepadX, selectedInput.GamepadY, out initialAngle, out initialStrength);
            ...
            stickAngleBox.Value = ClampDecimal(initialAngle, stickAngleBox.Minimum, stickAngleBox.Maximum);
            stickStrengthBox.Value = ClampDecimal(initialStrength, stickStrengthBox.Minimum, stickStrengthBox.Maximum);
        ...
        private static NumericUpDown MakeNumberBox(int min, int max, int value, int increment)
        {
        ...
        private void UpdateInputMode()
        {
            ...
            if (gamepad && selectedInput.Kind != InputKind.Gamepad)
            {
                ...
                gamepadControlBox.SelectedIndex = (int)GamepadControl.South;
                stickAngleBox.Value = 0;
                stickStrengthBox.Value = 100;
                triggerValueBox.Value = 100;
                loadingControls = false;
            }
            ...
            if (gamepad)
            {
                int selectedIndex = Math.Max(0, Math.Min(gamepadControlBox.Items.Count - 1, (int)selectedInput.GamepadControl));
                if (gamepadControlBox.SelectedIndex != selectedIndex)
                {
                    loadingControls = true;
                    gamepadControlBox.SelectedIndex = selectedIndex;
                    loadingControls = false;
                }
        ...
        private void UpdateGamepadControl()
        {
            ...
            stickAngleLabel.Visible = stick;
            stickAngleBox.Visible = stick;
            stickStrengthLabel.Visible = stick;
            stickStrengthBox.Visible = stick;
            triggerValueLabel.Visible = trigger;
            triggerValueBox.Visible = trigger;
            RefreshInputText();
        }

        private void GamepadVectorChanged(object sender, EventArgs e)
        {
            if (loadingControls || selectedInput == null || selectedInput.Kind != InputKind.Gamepad) return;
            GamepadVector.ToCartesian((int)stickAngleBox.Value, (int)stickStrengthBox.Value,
                out selectedInput.GamepadX, out selectedInput.GamepadY);
            RefreshInputText();
        }
        ...
        private void RefreshInputText()
        {
            ...
            if (gamepadPreview != null)
            {
                gamepadPreview.SetState(selectedInput.Kind == InputKind.Gamepad,
                    selectedInput.GamepadControl, (int)stickAngleBox.Value,
                    (int)stickStrengthBox.Value, (int)triggerValueBox.Value);
            }
        }
        ...
        private void Ok_Click(object sender, EventArgs e)
        {
            ...
            s.GamepadX = selectedInput.GamepadX;
            s.GamepadY = selectedInput.GamepadY;
            s.GamepadValue = selectedInput.GamepadValue;
    ...
    internal sealed class GamepadPreviewControl : Control
    {
        private bool active;
        private GamepadControl selectedControl;
        private int stickAngle;
        private int stickStrength;
        private int triggerStrength;

        public GamepadPreviewControl()
        {
        ...
        public void SetState(bool isActive, GamepadControl control, int angle, int strength, int trigger)
        {
            ...
            stickAngle = Math.Max(-180, Math.Min(180, angle));
            stickStrength = Math.Max(0, Math.Min(100, strength));
            triggerStrength = Math.Max(0, Math.Min(100, trigger));
            Invalidate();
        }
    ...
    {
        private sealed class HeldMappingRuntime
        {
        ...
        private int pendingInputStateRepairLogCount;
        ...
        internal MainForm(MacroConfig isolatedConfig, string isolatedDirectory) : this(isolatedConfig, isolatedDirectory, null) { }

        internal MainForm(MacroConfig isolatedConfig, string isolatedDirectory, IOutputBackend outputBackend)
        {
            outputOwnership = new OutputOwnershipManager(outputBackend ?? (IOutputBackend)new DirectOutputBackend());
            AutoScaleMode = AutoScaleMode.Dpi;
            AutoScaleDimensions = new SizeF(96F, 96F);
            ...
            idleGamepad = new IdleGamepadService(
                ...
                {
                    if (down) outputOwnership.SetDown(IdleOutputSourceId, input);
                    else outputOwnership.SetUp(IdleOutputSourceId, input);
                },
                delegate { return GamepadOutput.OwnXboxUserIndex; },
        ...
        private void BuildUi()
        {
            ...
            stepRunButton = MakeButton("▷ 单步执行", 0, 0, 0);
            stepRunButton.AutoSize = true;
            stepRunButton.MinimumSize = new Size(112, 38);
            stepRunButton.Margin = new Padding(0, 0, 0, 2);
            stepRunButton.Font = new Font(Font, FontStyle.Bold);
            stepRunButton.FlatStyle = FlatStyle.Flat;
            stepRunButton.FlatAppearance.BorderSize = 1;
            stepRunButton.FlatAppearance.BorderColor = SystemColors.ControlDark;
            stepRunButton.FlatAppearance.MouseOverBackColor = SystemColors.ControlLight;
            stepRunButton.FlatAppearance.MouseDownBackColor = SystemColors.ControlLight;
            stepRunButton.BackColor = SystemColors.Control;
            stepRunButton.UseVisualStyleBackColor = false;
            stepRunButton.Click += StepRunButton_Click;
            ...
            primaryActions.Controls.Add(stepRunButton);
        ...
        private void BuildToolsMenu()
        {
            ...
            runtimeObservationMenuItem = new ToolStripMenuItem();
            runtimeObservationMenuItem.Click += delegate
            {
                if (runtimeObservationForm != null && !runtimeObservationForm.IsDisposed)
                {
                    runtimeObservationForm.BringToFront();
                    runtimeObservationForm.Activate();
                    return;
                }
                runtimeObservationForm = new DiagnosticsForm(BuildRuntimeObservationText);
                runtimeObservationForm.Text = Localizer.IsEnglish ? AppInfo.ProductName + " - Runtime observation" : AppInfo.ProductName + " - 运行观察";
                runtimeObservationForm.FormClosed += delegate { runtimeObservationForm = null; };
                runtimeObservationForm.Show(this);
            };
        ...
        private string BuildRuntimeObservationText()
        {
            ...
            lock (runLock)
            {
                ...
                foreach (KeyValuePair<MacroDefinition, HeldMappingRuntime> pair in activeParallelHeldMappings)
                {
                    ...
                    if (runtime == null) continue;
                    string name = runtime.Macro == null ? "?" : runtime.Macro.Name;
                    heldMappings.Add(name + " | " + InputNames.FormatTrigger(runtime.Trigger) + " | " + runtime.SourceId);
                }
            }
            ...
            sb.AppendLine();
            sb.AppendLine((Localizer.IsEnglish ? "Parallel Held Mappings: " : "并行按住映射：") + heldMappings.Count.ToString());
            if (heldMappings.Count == 0) sb.AppendLine("  " + (Localizer.IsEnglish ? "None" : "无"));
            else foreach (string item in heldMappings) sb.AppendLine("  - " + item);

            OutputOwnershipSnapshot ownership = outputOwnership.Snapshot();
            sb.AppendLine();
            sb.AppendLine((Localizer.IsEnglish ? "Active output sources: " : "活动输出来源：") + ownership.Sources.Count.ToString() +
                "  Version=" + ownership.Version.ToString() + "  Reason=" + ownership.LastReason);
            foreach (OutputSourceSnapshot source in ownership.Sources)
            {
                sb.AppendLine("  [" + source.SourceId + "]" + (source.Suspended ? " SUSPENDED" : ""));
                foreach (InputSpec input in source.Contributions)
                    sb.AppendLine("    -> " + InputNames.FormatInput(input));
            }

            sb.AppendLine();
            sb.AppendLine((Localizer.IsEnglish ? "Merged output: " : "合并后输出：") + ownership.Merged.Count.ToString());
            if (ownership.Merged.Count == 0) sb.AppendLine("  " + (Localizer.IsEnglish ? "Neutral / released" : "全部中立 / 已释放"));
            else foreach (InputSpec input in ownership.Merged) sb.AppendLine("  = " + InputNames.FormatInput(input));
            return sb.ToString();
        }
        ...
        private string BuildDiagnosticsText()
        {
            ...
            OutputOwnershipSnapshot ownership = outputOwnership.Snapshot();
            sb.AppendLine("OutputOwnershipSources: " + ownership.Sources.Count.ToString());
            sb.AppendLine("MergedOutputs: " + ownership.Merged.Count.ToString());
            sb.AppendLine("OwnershipLastReason: " + ownership.LastReason);
        ...
        private void ReconcileHookState(string reason)
        {
            ...
            try
            {
                ...
                if (total > lastLoggedInputStateRepairCount)
                {
                    ...
                    pendingInputStateRepairLogCount += repaired;
                    bool periodic = string.Equals(reason, "foreground-timer", StringComparison.Ordinal);
                    if (!periodic || lastInputStateRepairLogUtc == DateTime.MinValue ||
                        (DateTime.UtcNow - lastInputStateRepairLogUtc).TotalSeconds >= 30)
                    {
                        AppLog.Write("Repaired stale physical input state (aggregated): +" + pendingInputStateRepairLogCount.ToString() +
                            ", total=" + total.ToString() + ", reason=" + (reason ?? "periodic"));
                        pendingInputStateRepairLogCount = 0;
                        lastInputStateRepairLogUtc = DateTime.UtcNow;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void ReconcileHeldTriggerRelease()
        {
            ...
            lock (runLock)
            {
                ...
                foreach (KeyValuePair<MacroDefinition, HeldMappingRuntime> pair in activeParallelHeldMappings)
                {
                    ...
                    else if (runtime.ReleaseProbeSince == 0)
                    {
                        runtime.ReleaseProbeSince = now;
                    }
                    else if (now - runtime.ReleaseProbeSince >= requiredTicks)
                    {
                        runtime.ReleaseProbeSince = 0;
        ...
        private void StartParallelHeldMappingWithTrigger(MacroDefinition macro, TriggerSpec triggerOverride)
        {
            ...
            lock (runLock)
            {
                ...
                runtime.ReleaseProbeSince = 0;
                activeParallelHeldMappings[macro] = runtime;
            }
            ...
            {
                HandleOutputOwnershipFailure(ex, "start-held");
            }
        }

        private void StopParallelHeldMapping(MacroDefinition macro, string reason)
        {
            if (macro == null) return;
            HeldMappingRuntime runtime = null;
            lock (runLock)
            {
                if (!activeParallelHeldMappings.TryGetValue(macro, out runtime)) return;
                activeParallelHeldMappings.Remove(macro);
            }
            try
            {
                outputOwnership.ClearSource(runtime.SourceId);
                runtimeTrace.Add("held-source-stop", "source=" + runtime.SourceId + "; reason=" + (reason ?? "release"));
            }
            catch (Exception ex)
            {
                HandleOutputOwnershipFailure(ex, "stop-held");
                return;
            }
            ...
            if (!HasWorkerBackedRuns())
            {
                ...
                statusLabel.Text = remaining == 0 ? Localizer.T("状态：空闲") :
                    (Localizer.IsEnglish ? "Held mappings active: " + remaining.ToString() : "按住映射活动：" + remaining.ToString());
            }
        }
        ...
        {
            List<HeldMappingRuntime> active;
            lock (runLock)
            {
                active = new List<HeldMappingRuntime>(activeParallelHeldMappings.Values);
                activeParallelHeldMappings.Clear();
            }
            foreach (HeldMappingRuntime runtime in active)
            {
                if (runtime == null || string.IsNullOrWhiteSpace(runtime.SourceId)) continue;
                try { outputOwnership.ClearSource(runtime.SourceId); }
                catch (Exception ex) { HandleOutputOwnershipFailure(ex, "stop-all-held"); break; }
            }
            if (active.Count != 0) runtimeTrace.Add("held-source-stop-all", "count=" + active.Count.ToString() + "; reason=" + (reason ?? "unknown"));
        }
        ...
        private void StepRunButton_Click(object sender, EventArgs e)
        {
            ...
            if (config.ActivateTargetWindowOnUiRun)
            {
                ...
                if (!NativeWindowFocus.TryActivate(target))
                {
                    statusLabel.Text = "状态：无法激活已锁定的目标窗口。请重新锁定目标或关闭界面运行时的目标切换。";
                    return;
                }
                startDelay = Math.Max(0, config.UiRunStartDelayMs);
            }
            ...
            {
                statusLabel.Text = Localizer.IsEnglish ? "Single-step started without switching windows." : "状态：开始单步执行（不切换窗口）。";
            }
        ...
        {
            StartMacro(m, startDelayMs, waitForReleaseTrigger, holdControlled, false);
        }
        ...
        private void MacroWorker(MacroRunRuntime runtime, int startDelayMs, TriggerSpec waitForReleaseTrigger)
        {
            ...
            if (singleStep) { infinite = false; repeatCount = 1; }
            ...
            try
            {
                ...
                if (WaitForUiSafetyClear(stop, macroName, held, sourceId)) return;
                ...
                while (!stop.IsSet)
                {
                    ...
                    for (int stepIndex = 0; stepIndex < steps.Count; stepIndex++)
                    {
                        ...
                        {
                            InputSpec existingHeld;
                            bool duplicateGamepadState = input.Kind == InputKind.Gamepad &&
                                held.TryGetValue(key, out existingHeld) && SameGamepadState(existingHeld, input);
                        ...
                        if (!singleStep)
                        {
                            ...
                            if (WaitOrStopWithUiSafety(stop, stepDelay, macroName, held, sourceId)) break;
                        }
                    }
            ...
            catch (Exception ex)
            {
                ...
                if (ex is OutputOwnershipException) HandleOutputOwnershipFailure(ex, "macro-worker");
            }
            finally
            {
        ...
        private bool WaitForSingleStepAdvance(MacroRunRuntime runtime, int completedSteps, int totalSteps)
        {
            ...
            while (!stop.IsSet)
            {
                if (gate != null && gate.WaitOne(50)) break;
                if (gate == null && stop.Wait(50)) break;
            }
        ...
        private bool WaitForUiSafetyClear(ManualResetEventSlim stop, string macroName, Dictionary<string, InputSpec> held, string sourceId)
        {
            ...
            // Suspend only this worker's source. Other parallel Held Mapping sources remain active.
            try { outputOwnership.SuspendSource(sourceId); }
            catch (OutputOwnershipException ex) { HandleOutputOwnershipFailure(ex, "ui-suspend"); return true; }
            ...
            try { outputOwnership.ResumeSource(sourceId); }
            catch (OutputOwnershipException ex) { HandleOutputOwnershipFailure(ex, "ui-resume"); return true; }
            SetStatusSafe("正在执行：" + macroName + "（UI编辑保护已恢复）");
            return false;
        }

        private bool WaitOrStopWithUiSafety(ManualResetEventSlim stop, int ms, string macroName, Dictionary<string, InputSpec> held, string sourceId)
        {
        ...
        private bool WaitForDangerousPhysicalShortcutToClear(ManualResetEventSlim stop, string macroName, Dictionary<string, InputSpec> held, MacroStep step, string sourceId)
        {
            ...
            while (!stop.IsSet)
            {
                if (uiSafetyPauseRequested && WaitForUiSafetyClear(stop, macroName, held, sourceId)) return true;
        ...
        private static bool SameGamepadState(InputSpec left, InputSpec right)
        {
            if (left == null || right == null || left.Kind != InputKind.Gamepad || right.Kind != InputKind.Gamepad) return false;
            return left.GamepadControl == right.GamepadControl &&
                left.GamepadX == right.GamepadX && left.GamepadY == right.GamepadY &&
                left.GamepadValue == right.GamepadValue;
        }
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            try
            {
                ...
                outputOwnership.ClearAll("shutdown");
                GamepadOutput.NeutralizeAll();
                GamepadOutput.Disconnect();
                SaveConfig();
                if (hooks != null)
