...
- 修复配置备份保留顺序在极快连续保存时可能不稳定的问题：Windows 墙钟分辨率可能让多个备份得到同一时间串，旧逻辑随后会退化成按随机 GUID 排序。现在有效配置备份使用严格单调递增的排序时间戳，即使时钟粒度很粗、短暂回拨或程序重启，也会稳定保留真正最近的 5 个版本。
...
- 原有 323 项键盘测试、58 项闲置手柄测试、26 项 XInput 输入测试、28 项多手柄汇总测试、27 项 0 号槽位取得测试、15 项虚拟手柄偏好测试、7 项宏计时测试、43 项更新安装测试、18 项更新网络测试、23 项界面安全/诊断测试、**360 项配置/易用性测试**和 **303,716 项输出合并/并发测试**继续通过。
