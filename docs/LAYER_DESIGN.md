...
Status: **designed, intentionally not enabled in 1.3.0-beta.1**.

状态：**已完成设计，但有意不在 1.3.0-beta.1 中启用。**
...
Layer should initially scope **Held Mapping only**. Ordinary timed macros remain global and single-active.

Layer 第一版只管理 **Held Mapping**；普通时序宏继续保持全局、最多一个运行实例。
...
Layer implementation should start only after Beta 1 passes real-use acceptance for:

- simultaneous WASD stick mapping;
- simultaneous trigger/button mappings;
- ordinary timed macro + Held Mapping coexistence;
- release order permutations;
- Alt+Tab / lost-KeyUp recovery;
- Emergency Stop;
- repeated start/stop without residual virtual-controller state.

Only then should Layer be implemented as a thin eligibility/grouping layer on top of the existing ownership runtime, not as a second execution engine.

只有 Beta 1 在真实使用中通过上述验收后，Layer 才应作为现有 Ownership Runtime 之上的“资格筛选/分组层”实现，而不是再造第二套执行引擎。
