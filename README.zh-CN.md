...
**当前正式版：v1.2.0。** 这是 1.1.1 Beta 1–4 经过实际使用与回归验证后的稳定晋升版。后续较大功能仍会先进入公开 Beta，再经过实际使用后决定是否晋升 Stable。Beta 与正式版继续使用独立发布通道，Beta 不替换正式版更新清单。

Beta 与正式版目前共用配置。测试未来 Beta 前请退出另一版本并备份 `%APPDATA%\InputStitch`。详见[开发路线和发布规则](ROADMAP.md)。
...
录制宏。录制器只录制物理键鼠；虚拟手柄步骤需手动添加。“录制宏”。录制器只录制物理键鼠；虚拟手柄步骤需手动添加。
4. 录制触发键并选择触发方式。
